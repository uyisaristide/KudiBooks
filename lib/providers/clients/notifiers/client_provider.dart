...
class ClientNotifier extends StateNotifier<NetworkInfo<List<ClientModel>>> {
  ...
  Future<NetworkInfo> registerClient(ClientModel client) async {
    ...
    try {
      ...
      var clients = NetworkInfo<List<ClientModel>>(
      ...
      NetworkInfo<List<ClientModel>> info = NetworkInfo(errorMessage: "Contact system admin", networkStatus: NetworkStatus.error);
      state=info;
      return info;
    }
  }
  ...
  Future<NetworkInfo> removeClient(int? id) async {
    ...
    try {
      ...
      Response response = await _dio.delete(
          '${DioServices.baseUrl}app/client/delete/$id',
          options: Options(headers: chartHeaders));
      var info = NetworkInfo<List<ClientModel>>(
          networkStatus: NetworkStatus.success, statusCode: 200);
      state = info;
      return info;
    } on DioError catch (e) {
  ...
  Future<NetworkInfo> editClient(
      {required ClientModel client, required int id}) async {
    state = NetworkInfo(networkStatus: NetworkStatus.loading);
    try {
      Map<String, dynamic> newClientHeader = {
        "Authorization": "Bearer  ${Hive.box('tokens').get('token')}",
      ...
      debugPrint("${client.clientToJson()}");
      var responses = await _dio.put(
          "${DioServices.baseUrl}app/client/update/$id",
          data: client.clientToJson(),
          options: Options(headers: newClientHeader));
